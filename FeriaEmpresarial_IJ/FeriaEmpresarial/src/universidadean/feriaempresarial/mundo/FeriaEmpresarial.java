...
public class FeriaEmpresarial {
     ...
     * @return respuesta1
     */
    int cantPuestosZonaSur = 0;
	int cantPuestosZonaOriente = 0;
	int cantPuestosZonaNorte = 0;
	int cantPuestosZonaOccidente = 0;
	int cantPuestosZonaCentro = 0;
    	
    public String metodo1() {
    	
    	
    // CODIGO PARA GUARDAR LA CANTIDAD DE PUESTOS OCUPADOS EN CADA ZONA
    	...
    	for(Puesto p : puestos) {
    		if(p.estaOcupado() && p.darZona().equals(Puesto.ZONA_NORTE)) {
    			cantPuestosZonaNorte = cantPuestosZonaNorte + 1;
    		...
    		if(p.estaOcupado() && p.darZona().equals(Puesto.ZONA_ORIENTE)) {
    			cantPuestosZonaOriente = cantPuestosZonaOriente + 1;
    			
    		}	
    		
    			
    		if(p.estaOcupado() && p.darZona().equals(Puesto.ZONA_SUR)) {
    			cantPuestosZonaSur = cantPuestosZonaSur + 1;
    			
    		}
    		
    			
    		if(p.estaOcupado() && p.darZona().equals(Puesto.ZONA_OCCIDENTE)) {
        		cantPuestosZonaOccidente = cantPuestosZonaOccidente + 1;
        		
        	}	
        		
        	if(p.estaOcupado() && p.darZona().equals(Puesto.ZONA_CENTRO)) {
        		cantPuestosZonaCentro = cantPuestosZonaCentro + 1;
    	...
    	// CODIGO PARA DECIR Y COMPARAR QUE ZONA TIENE MAS PUESTOS
    	
    		
    	
    	
    	// ZONA NORTE
    	if(cantPuestosZonaNorte > cantPuestosZonaOriente && cantPuestosZonaNorte > cantPuestosZonaSur && cantPuestosZonaNorte > cantPuestosZonaOccidente && cantPuestosZonaNorte > cantPuestosZonaCentro ) {
    		return("La " + Puesto.ZONA_NORTE +" es la que tiene mas puestos ocupados");
    	}
    ...
    // ZONA ORIENTE
    	...
    	if(cantPuestosZonaOriente > cantPuestosZonaNorte && cantPuestosZonaOriente > cantPuestosZonaSur && cantPuestosZonaOriente > cantPuestosZonaOccidente && cantPuestosZonaOriente > cantPuestosZonaCentro ) {
    		return("La " + Puesto.ZONA_ORIENTE +" es la que tiene mas puestos ocupados");
    	}
    	
	    	
	//ZONA SUR
    	
    	if(cantPuestosZonaSur > cantPuestosZonaNorte && cantPuestosZonaSur > cantPuestosZonaOriente && cantPuestosZonaSur > cantPuestosZonaOccidente && cantPuestosZonaSur > cantPuestosZonaCentro ) {
    		return("La " + Puesto.ZONA_SUR +" es la que tiene mas puestos ocupados");
    	}
    	...
    	if(cantPuestosZonaOccidente > cantPuestosZonaNorte && cantPuestosZonaOccidente > cantPuestosZonaOriente && cantPuestosZonaOccidente > cantPuestosZonaSur && cantPuestosZonaOccidente > cantPuestosZonaCentro ) {
    		return("La " + Puesto.ZONA_OCCIDENTE +" es la que tiene mas puestos ocupados");
    	}
    	
    	
    		//ZONA CENTRO
    	
    	if(cantPuestosZonaCentro > cantPuestosZonaNorte && cantPuestosZonaCentro > cantPuestosZonaOriente && cantPuestosZonaCentro > cantPuestosZonaSur && cantPuestosZonaCentro > cantPuestosZonaOccidente ) {
    		return("La " + Puesto.ZONA_CENTRO +" es la que tiene mas puestos ocupados");
    	}
    	
    	
    	
		return ("Hay la misma cantidad de zonas ocupadas o no hay zonas ocupadas");
    ...
    public  String metodo2() {
      ...
      //  return "holi si sirvio " + df.format(porcentaje) + "%";
        
        if(porcentaje == 60 ) {
			return "se llego al punto de equilibrio " +  df.format(porcentaje) + "%";
		}
        
        else if (porcentaje < 60 ) {
        	return "aun no se llega al punto de equilibrio " +  df.format(porcentaje) + "%";
        }
